...
NodeSocketServer.prototype._clientDataReceived = function(client, socket, buffer) {
	...
	if(client._state === NodeSocketCommon.EnumConnectionState.Connected) {
		if(buffer.slice(0, NodeSocketCommon.nodesocketSignature.length).equals(NodeSocketCommon.nodesocketSignature)) {
			...
			// Fixes bug appears when a remote client is able to write function payloads to the sockets data stream faster than
			// the NodeJS engine can recongnize the difference in buffer data, thus losing the following payload.
			if(buffer.length > 1) {
				this._clientDataReceived(client, socket, buffer.slice(1));
			}
		}
		else {
...
NodeSocketServer.prototype.stop = function() {
	if(this._server) {
		var self = this;
