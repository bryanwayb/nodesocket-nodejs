...
NodeSocketServer.prototype._nodeWebSocketDataReceived = function(client, socket, buffer) {
	...
	if(client._state === NodeSocketCommon.EnumConnectionState.WebSocketConnected) {
		...
		// Don't use the client._write wrapper here, we need full control over the formatting
		...
		if(frame.opcode === 0x9) {
			frame.opcode = 0xA;
			client._write(NodeSocketCommon.makeWebSocketFrame(frame));
		}
			...
			this._buffer += frame.payload.toString(NodeSocketCommon.encoding);
			
			if(frame.final) {
				this._nodeDataReceived(client, socket, new Buffer(this._buffer, NodeSocketCommon.encoding));
				this._buffer = '';
			}
		}
	}
...
NodeSocketServer.prototype._nodeDataReceived = function(client, socket, buffer) {
	...
	if(client._state === NodeSocketCommon.EnumConnectionState.Connected) {
		if(NodeSocketCommon.nodesocketSignature.equals(buffer)) {
			...
			client._write(NodeSocketCommon.nodesocketSignature); // Send same verification string back to the client
		...
		if(client._master) {
			if(execCode === NodeSocketCommon.EnumExecutionCode.RequestMaster) {
				...
				client._write(new Buffer([ NodeSocketCommon.EnumNodeResponse.NotAllowed ]));
			}
		}
		else {
			if(execCode === NodeSocketCommon.EnumExecutionCode.ExecFunction) {
				...
				var writeBuffer = NodeSocketCommon.parseExecutePayload.call(client, this._functions, buffer.slice(1), function(error) {
					self.emit('error', error, client, socket);
				});
				
				bufferPosition = 1 + writeBuffer.position;
				
				client._write(writeBuffer.value);
			}
			else if(execCode === NodeSocketCommon.EnumExecutionCode.RequestSlave) {
				...
				client._write(new Buffer([ NodeSocketCommon.EnumNodeResponse.InvalidExecCode ]));
			}
		}
	}
	
	if(bufferPosition < buffer.length) {
		this._nodeDataReceived(client, socket, buffer.slice(bufferPosition));
	}
};
