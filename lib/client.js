	...
	if(this._state === NodeSocketCommon.EnumConnectionState.Connected) {
		...
		if(this._master) {
			if(execCode === NodeSocketCommon.EnumExecutionCode.RequestMaster) {
				...
				this._socket.write(new Buffer([ NodeSocketCommon.EnumNodeResponse.NotAllowed ]));
			}
		}
		else {
			if(execCode === NodeSocketCommon.EnumExecutionCode.ExecFunction) {
				var self = this;
				this._socket.write(NodeSocketCommon.parseExecutePayload(this._functions, buffer.slice(1), function(error) {
					self.emit('error', error, this._socket);
				}));
			}
				...
				this._socket.write(new Buffer([ NodeSocketCommon.EnumNodeResponse.InvalidExecCode ]));
			}
		}
	}
	else if(this._state === NodeSocketCommon.EnumConnectionState.Processing) {
		...
		if(nodeResponse === NodeSocketCommon.EnumNodeResponse.Okay) { // No error reported by node
			this._processCallback(NodeSocketCommon.parseResultPayload(buffer.slice(1)));
		}
			...
			if(nodeResponse in NodeSocketCommon.EnumNodeResponseErrorString) {
				this.emit('error', NodeSocketCommon.EnumNodeResponseErrorString[nodeResponse], this._socket);
			}
			else {
